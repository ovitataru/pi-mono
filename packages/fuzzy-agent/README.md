...
Stateful agent with tool execution and event streaming. Built on `@fuzzyos/fuzzy-ai`.

## Installation

```bash
npm install @fuzzyos/fuzzy-agent
```
...
```typescript
import { Agent, streamProxy } from "@fuzzyos/fuzzy-agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@fuzzyos/fuzzy-agent";
