 ...
 *
 * Place this file at: packages/fuzzy-tui/test/viewport-overwrite-repro.ts
 * Run from repo root: npx tsx packages/fuzzy-tui/test/viewport-overwrite-repro.ts
 *
 * For reliable repro, run in a small terminal (8-12 rows) or a tmux session:
 *   tmux new-session -d -s tui-bug -x 80 -y 12
 *   tmux send-keys -t tui-bug "npx tsx packages/fuzzy-tui/test/viewport-overwrite-repro.ts" Enter
