...
describe("CombinedAutocompleteProvider", () => {
	...
	describe("fd @ file suggestions", { skip: !isFdInstalled }, () => {
		...
		test("matches deeply nested paths", async () => {
			setupFolder(baseDir, {
				files: {
					"packages/fuzzy-tui/src/autocomplete.ts": "export {};",
					"packages/fuzzy-ai/src/autocomplete.ts": "export {};",
				},
			});
			...
			const values = result?.items.map((item) => item.value);
			assert.ok(values?.includes("@packages/fuzzy-tui/src/autocomplete.ts"));
			assert.ok(!values?.includes("@packages/fuzzy-ai/src/autocomplete.ts"));
		});
