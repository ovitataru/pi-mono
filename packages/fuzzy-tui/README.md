...
```typescript
import { TUI, Text, Editor, ProcessTerminal } from "@fuzzyos/fuzzy-tui";
...
```typescript
import { CombinedAutocompleteProvider } from "@fuzzyos/fuzzy-tui";
...
```typescript
import { visibleWidth, truncateToWidth, wrapTextWithAnsi } from "@fuzzyos/fuzzy-tui";
...
```typescript
import { matchesKey, Key, truncateToWidth } from "@fuzzyos/fuzzy-tui";
...
```bash
FUZZY_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx test/chat-simple.ts
```
