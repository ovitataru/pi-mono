...
```bash
npm install @fuzzyos/fuzzy-ai
```

TypeBox exports are re-exported from `@fuzzyos/fuzzy-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@fuzzyos/fuzzy-ai';
...
```typescript
import { Type, Tool, StringEnum } from '@fuzzyos/fuzzy-ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@fuzzyos/fuzzy-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@fuzzyos/fuzzy-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@fuzzyos/fuzzy-ai';
...
```typescript
import { getModel, stream } from '@fuzzyos/fuzzy-ai';
...
```typescript
import { getProviders, getModels, getModel } from '@fuzzyos/fuzzy-ai';
...
```typescript
import { Model, stream } from '@fuzzyos/fuzzy-ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@fuzzyos/fuzzy-ai';
...
```typescript
import { getModel, complete, Context } from '@fuzzyos/fuzzy-ai';
...
```typescript
import { Context, getModel, complete } from '@fuzzyos/fuzzy-ai';
...
- OAuth login flows are not supported in browser environments. Use the `@fuzzyos/fuzzy-ai/oauth` entry point in Node.js.
...
Set `FUZZY_AI_ANTIGRAVITY_VERSION` to override the Antigravity User-Agent version when Google updates their requirements:

```bash
export FUZZY_AI_ANTIGRAVITY_VERSION="1.23.0"
```

#### Cache Retention

Set `FUZZY_CACHE_RETENTION=long` to extend prompt cache retention:

| Provider | Default | With `FUZZY_CACHE_RETENTION=long` |
|----------|---------|-------------------------------|
...
```typescript
import { getEnvApiKey } from '@fuzzyos/fuzzy-ai';
...
```bash
npx @fuzzyos/fuzzy-ai login              # interactive provider selection
npx @fuzzyos/fuzzy-ai login anthropic    # login to specific provider
npx @fuzzyos/fuzzy-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@fuzzyos/fuzzy-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@fuzzyos/fuzzy-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@fuzzyos/fuzzy-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@fuzzyos/fuzzy-ai`
...
Update `packages/fuzzy-ai/README.md`:
...
Add an entry to `packages/fuzzy-ai/CHANGELOG.md` under `## [Unreleased]`:
