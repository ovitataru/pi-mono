...
describe("Cache Retention (FUZZY_CACHE_RETENTION)", () => {
	const originalEnv = process.env.FUZZY_CACHE_RETENTION;

	beforeEach(() => {
		delete process.env.FUZZY_CACHE_RETENTION;
	});

	afterEach(() => {
		if (originalEnv !== undefined) {
			process.env.FUZZY_CACHE_RETENTION = originalEnv;
		} else {
			delete process.env.FUZZY_CACHE_RETENTION;
		}
	});
	...
	describe("Anthropic Provider", () => {
		it.skipIf(!process.env.ANTHROPIC_API_KEY)(
			"should use default cache TTL (no ttl field) when FUZZY_CACHE_RETENTION is not set",
			async () => {
		...
		it.skipIf(!process.env.ANTHROPIC_API_KEY)("should use 1h cache TTL when FUZZY_CACHE_RETENTION=long", async () => {
			process.env.FUZZY_CACHE_RETENTION = "long";
			const model = getModel("anthropic", "claude-3-5-haiku-20241022");
			let capturedPayload: any = null;
	...
	describe("OpenAI Responses Provider", () => {
		it.skipIf(!process.env.OPENAI_API_KEY)(
			"should not set prompt_cache_retention when FUZZY_CACHE_RETENTION is not set",
			async () => {
		...
		it.skipIf(!process.env.OPENAI_API_KEY)(
			"should set prompt_cache_retention to 24h when FUZZY_CACHE_RETENTION=long",
			async () => {
				process.env.FUZZY_CACHE_RETENTION = "long";
				const model = getModel("openai", "gpt-4o-mini");
				let capturedPayload: any = null;
