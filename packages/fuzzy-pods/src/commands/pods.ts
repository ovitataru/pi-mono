...
export const setupPod = async (
	...
	const vllmApiKey = process.env.FUZZY_API_KEY;
	...
	if (!vllmApiKey) {
		console.error(chalk.red("ERROR: FUZZY_API_KEY environment variable is required"));
		console.error("Set an API key: export FUZZY_API_KEY=your_api_key_here");
		process.exit(1);
	}
	...
	console.log(chalk.cyan(`  fuzzy start <model> --name <name>`));
};
