...
chmod +x /tmp/model_run_${name}.sh`,
	...
	const env = [
		...
		`FUZZY_API_KEY='${process.env.FUZZY_API_KEY}'`,
...
WRAPPER
	...
	if (startupFailed) {
		...
		console.log(`\n${chalk.cyan(`Check full logs: fuzzy ssh "tail -100 ~/.vllm_logs/${name}.log"`)}`);
		process.exit(1);
		...
		console.log(chalk.white("API Key:     ") + chalk.yellow(process.env.FUZZY_API_KEY || "(not set)"));
		console.log(chalk.cyan("─".repeat(50)));

		console.log(`\n${chalk.bold("Export for shell:")}`);
		console.log(chalk.gray(`export OPENAI_BASE_URL="http://${host}:${port}/v1"`));
		console.log(chalk.gray(`export OPENAI_API_KEY="${process.env.FUZZY_API_KEY || "your-api-key"}"`));
  ...
  curl $OPENAI_BASE_URL/chat/completions \\
    ...
    -d '{"model":"${modelId}","messages":[{"role":"user","content":"Hi"}]}'`),
		...
		console.log("");
		console.log(chalk.cyan(`Chat with model:  fuzzy agent ${name} "Your message"`));
		console.log(chalk.cyan(`Interactive mode: fuzzy agent ${name} -i`));
		console.log(chalk.cyan(`Monitor logs:     fuzzy logs ${name}`));
		console.log(chalk.cyan(`Stop model:       fuzzy stop ${name}`));
	} else if (interrupted) {
		console.log(chalk.yellow("\n\nStopped monitoring. Model deployment continues in background."));
		console.log(chalk.cyan(`Chat with model: fuzzy agent ${name} "Your message"`));
		console.log(chalk.cyan(`Check status: fuzzy logs ${name}`));
		console.log(chalk.cyan(`Stop model: fuzzy stop ${name}`));
	} else {
...
export const listModels = async (options: { pod?: string }) => {
	...
	if (anyDead) {
		...
		console.log(chalk.cyan("  fuzzy stop <name>"));
	} else {
...
export const showKnownModels = async () => {
	...
	console.log("Usage: fuzzy start <model> --name <name> [options]\n");
