...
Pod Management:
  fuzzy pods setup <name> "<ssh>" --mount "<mount>"    Setup pod with mount command
    Options:
  ...
  fuzzy pods                                           List all pods (* = active)
  fuzzy pods active <name>                             Switch active pod
  fuzzy pods remove <name>                             Remove pod from local config
  fuzzy shell [<name>]                                 Open shell on pod (active or specified)
  fuzzy ssh [<name>] "<command>"                       Run SSH command on pod
...
Model Management:
  ...
  fuzzy stop [<name>]                                  Stop model (or all if no name)
  fuzzy list                                           List running models
  fuzzy logs <name>                                    Stream model logs
  fuzzy agent <name> ["<message>"...] [options]        Chat with model using agent & tools
  fuzzy agent <name> [options]                         Interactive chat mode
...
Environment:
  ...
  FUZZY_API_KEY     API key for vLLM endpoints
  FUZZY_CONFIG_DIR    Config directory (default: ~/.pi)`);
}
...
try {
	...
	if (command === "pods") {
		if (!subcommand) {
			// fuzzy pods - list all pods
			listPods();
		} else if (subcommand === "setup") {
			// fuzzy pods setup <name> "<ssh>" [--mount "<mount>"] [--models-path <path>] [--vllm release|nightly|gpt-oss]
			const name = args[2];
			const sshCmd = args[3];

			if (!name || !sshCmd) {
				console.error(
					'Usage: fuzzy pods setup <name> "<ssh>" [--mount "<mount>"] [--models-path <path>] [--vllm release|nightly|gpt-oss]',
				);
				process.exit(1);
			}
			...
			// fuzzy pods active <name>
			const name = args[2];
			if (!name) {
				console.error("Usage: fuzzy pods active <name>");
				process.exit(1);
			}
			switchActivePod(name);
		} else if (subcommand === "remove") {
			// fuzzy pods remove <name>
			const name = args[2];
			if (!name) {
				console.error("Usage: fuzzy pods remove <name>");
				process.exit(1);
			}
		...
		switch (command) {
			case "shell": {
				// fuzzy shell [<name>] - open interactive shell
			...
			case "ssh": {
				// fuzzy ssh [<name>] "<command>" - run command via SSH
				let podName: string | undefined;
				let sshCommand: string;

				if (args.length === 2) {
					// fuzzy ssh "<command>" - use active pod
					sshCommand = args[1];
				} else if (args.length === 3) {
					// fuzzy ssh <name> "<command>"
					podName = args[1];
					sshCommand = args[2];
				} else {
					console.error('Usage: fuzzy ssh [<name>] "<command>"');
					process.exit(1);
				}
			...
			case "start": {
				// fuzzy start <model> --name <name> [options]
			...
			case "stop": {
				// fuzzy stop [name] - stop specific model or all models
				const name = args[1];
				if (!name) {
			...
			case "logs": {
				...
				const name = args[1];
				if (!name) {
					console.error("Usage: fuzzy logs <name>");
					process.exit(1);
				}
			...
			case "agent": {
				// fuzzy agent <name> [messages...] [options]
				const name = args[1];
				if (!name) {
					console.error("Usage: fuzzy agent <name> [messages...] [options]");
					process.exit(1);
				}

				const apiKey = process.env.FUZZY_API_KEY;
