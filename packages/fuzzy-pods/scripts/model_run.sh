#!/usr/bin/env bash
# Model runner script - runs sequentially, killed by fuzzy stop
set -euo pipefail

# These values are replaced before upload by fuzzy CLI
MODEL_ID="{{MODEL_ID}}"
NAME="{{NAME}}"
PORT="{{PORT}}"
VLLM_ARGS="{{VLLM_ARGS}}"
...
# Build vLLM command
VLLM_CMD="vllm serve '$MODEL_ID' --port $PORT --api-key '$FUZZY_API_KEY'"
if [ -n "$VLLM_ARGS" ]; then
