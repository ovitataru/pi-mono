...
Fuzzy automates vLLM deployment on GPU pods from DataCrunch, Vast.ai, Prime Intellect, RunPod (or any Ubuntu machine with NVIDIA GPUs). It manages multiple concurrent model deployments via separate vLLM instances, each accessible through the OpenAI API protocol with API key authentication.

Pods are treated as ephemeral - spin up when needed, tear down when done. To avoid re-downloading models (30+ minutes for 100GB+ models), fuzzy uses persistent network volumes for model storage that can be shared across pods on the same provider. This minimizes both cost (only pay for active compute) and setup time (models already cached).

## Usage

### Pods
```bash
pi pods setup dc1 "ssh root@1.2.3.4" --mount "mount -t nfs..."  # Setup pod (requires HF_TOKEN, FUZZY_API_KEY env vars)
...
### Models
```bash
pi start Qwen/Qwen2.5-72B-Instruct --name qwen72b          # Known model - fuzzy handles vLLM args
...
For known models, fuzzy automatically configures appropriate vLLM arguments from model documentation based on the hardware of the pod. For unknown models or custom configurations, pass vLLM args after `--vllm`.

## Pod management

Fuzzy manages GPU pods from various providers (DataCrunch, Vast.ai, Prime Intellect, RunPod) as ephemeral compute resources. Users manually create pods via provider dashboards, then register them with fuzzy for automated setup and management.
...
When a user creates a fresh pod on a provider, they register it with fuzzy using the SSH command from the provider:
...
- `FUZZY_API_KEY`: API key for securing vLLM endpoints
...
Fuzzy uses direct SSH commands to manage vLLM instances on pods. No remote manager component is needed - everything is controlled from the local fuzzy CLI.

### Architecture
The fuzzy CLI maintains all state locally in `~/.pi/pods.json`:
```json
{
  "pods": {
    "dc1": {
...
The location of the fuzzy config dir can also be specified via the `FUZZY_CONFIG_DIR` env var, e.g. for testing.

Pods are assumed to be fully managed by fuzzy - no other processes compete for ports or GPUs.
...
4. Builds vLLM command with appropriate args and FUZZY_API_KEY
